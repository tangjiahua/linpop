...
int editProfile(char *buf, int confd);
int editPassword(char *buf, int confd);
...
void * handClient(void *arg)
{
	...
	while(1)
	{
		...
		switch(buf[0])
		{
			...
			// 	break;
			case '8':
				editProfile(buf, confd);
				break;
			case 'c':
				editPassword(buf, confd);
				break;
...
int editProfile(char *buf, int confd){
	...
	sscanf(buf+2,"%[^|]|%[^|]|%[^|]|%[^|]",uName, head, sig, inv);
	sprintf(sqlStr, "%s '%s', %s '%s' %s'%s';", "update information set phaddr = ", head, " autograph = ", sig, "where account = ", uName);
	...
	if(mysql_query(&mysql, sqlStr) != 0){
		...
		memset(sqlStr, 0, sizeof(sqlStr));
		sprintf(sqlStr,"%s '%s' %s '%s';", "update userlist set invcode = ", inv, "where account =  ", uName);
		printf("%s\n", sqlStr);
		if(mysql_query(&mysql, sqlStr) != 0){
			//插入失败，未知原因
			send(confd,"0",strlen("0"),0);
			...
			char editReturn[30];
			memset(editReturn, 0, sizeof(editReturn));
			strcat(editReturn, "3|");
			strcat(editReturn, uName);
			strcat(editReturn, "|");
			strcat(editReturn, head);
			strcat(editReturn, "|");
			strcat(editReturn, sig);
			strcat(editReturn, "|");
			strcat(editReturn, "1");
			send(confd, editReturn, sizeof(editReturn), 0);
		}
...
int editPassword(char *buf, int confd){
	...
	sscanf(buf+2,"%[^|]|%[^|]",uName, newPassword);
	sprintf(sqlStr, "%s '%s' %s '%s';", "update userlist set password = ", newPassword, " where account = ",uName);
